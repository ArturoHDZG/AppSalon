...
2. Create a new database with the name `appsalon_db`.
3. Select the newly created database and run the `appsalon_db.sql` file to import the data.

### Running the Project

1. Serve the project by navigating to the `/public` directory and running the command `php -S localhost:3000`.
2. In a separate terminal, run the command `npx gulp` in Windows to compile SASS and execute other dependencies.
